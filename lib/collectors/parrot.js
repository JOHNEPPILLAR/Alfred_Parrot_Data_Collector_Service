...
const UUID_CHARACTERISTIC_SERIAL_NUMBER = '2a25';
...
async function discoverDevices() {
  ...
  noble.on('discover', async (peripheral) => {
    ...
    if (currentDevice.length === 0) {
      this.logger.info(`Found device not assigned to garden: ${peripheral.id}`);
      return;
    }
...
async function getDevices() {
  ...
  try {
    ...
    debug(`${this.devices.length} device(s) to discover and process`);
  } catch (err) {
    this.logger.error(`${this._traceStack()} - ${err.message}`);
  } finally {
    try {
